import { ALLERGENS, GLUTEN_STATUSES } from './constants.js';
...
// Görseller Firestore belgesinin içinde küçültülmüş data URL olarak saklanır (Storage gerektirmez).
export const MAX_IMAGE_CHARS = 300000;
function image(v) {
  const s = String(v ?? '').trim();
  if (!s) return '';
  if (/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(s) && s.length <= MAX_IMAGE_CHARS) return s;
  if (/^https:\/\/[^\s"'<>]{1,500}$/i.test(s)) return s;
  return '';
}

export class ValidationError extends Error {}

export function cleanProduct(input, db) {
  const name = str(input.name, 120);
  if (!name) throw new ValidationError('Ürün adı zorunlu');
  if (!db.categories.some((c) => c.id === input.categoryId)) throw new ValidationError('Geçerli bir kategori seçin');
...
export function cleanCategory(input) {
  ...
  if (!name) throw new ValidationError('Kategori adı zorunlu');
  return { name, active: bool(input.active, true) };
}

export function cleanTag(input) {
  const label = str(input.label, 30);
  if (!label) throw new ValidationError('Etiket adı zorunlu');
...
export function cleanSettings(input, current) {
  ...
  return {
    ...
    initialPasswordChanged: bool(input.initialPasswordChanged, !!current.initialPasswordChanged),
  };
}
