import { DEFAULT_TAGS, DEFAULT_SETTINGS } from './constants.js';

// ercanbrgr.com/menu/ sayfasındaki kategori ve ürünler.
// Kalori / alerjen / glutensiz bilgileri BİLEREK boş bırakıldı: bu veriler gıda güvenliğini
// ilgilendirir, admin panelinden işletme tarafından girilmelidir.
const MENU = [
...
// uid: benzersiz kısa kimlik üreten fonksiyon
export function buildSeed(uid) {
  const categories = [];
  const products = [];
  let order = 0;
  ...
  return { settings: { ...DEFAULT_SETTINGS }, categories, tags: DEFAULT_TAGS.map((t) => ({ ...t })), products };
}
